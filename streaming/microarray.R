...
colon %>% mutate(target=as.numeric(as.factor(y))) %>% select(target) %>% write_csv("colon_train.labels")

prostate %>% write_csv("prostate.csv")
prostate %>% select(-y) %>% write_csv("prostate_train.csv")
prostate %>% mutate(target=as.numeric(as.factor(y))) %>% select(target) %>% write_csv("prostate_train.labels")

leukemia %>% write_csv("leukemia.csv")
leukemia %>% select(-y) %>% write_csv("leukemia_train.csv")
leukemia %>% mutate(target=as.numeric(as.factor(y))) %>% select(target) %>% write_csv("leukemia_train.labels")

lung_cancer %>% write_csv("lung_cancer.csv")
lung_cancer %>% select(-y) %>% write_csv("lung_cancer_train.csv")
lung_cancer %>% mutate(target=as.numeric(as.factor(y))) %>% select(target) %>% write_csv("lung_cancer_train.labels")
