...
def class_separability(X, y, mode='mitra'):
    ...
    S_c1 = np.trace(s_b)/np.trace(s_w) # OGFS (criterion1) 
    ...
    def _dpp_sel(self, X_, y=None):
        ...
        *  `self.coef_info['cols']`
        ...
        for idx, feat in enumerate(X_sel.T):
            if len(feat_classsep) == 0:
                feat_classsep.append(idx)
                continue
            if class_separability(X_sel[:, feat_classsep], y) > class_separability(X_sel[:, feat_classsep+[idx]], y):
                feat_classsep.append(idx)
            else:
                excl_classsep.append(idx)
        ...
        if not self.unseen_only:
            ...
            index_to_col = [col for idx, col in enumerate(X_.columns) if idx in (feat_check+feat_classsep)]
        else:
